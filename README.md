...
Are you tired of `ASLR` getting in your way? Annoyed by how unexploitable mmap chunks feel in GLibC's `malloc`?   
...
This technique and was first used in a Qualys exploit for a [15 year old QMail vulnerability](https://lwn.net/Articles/820969). Another example of this being used is in [cpio](https://github.com/fangqyi/cpiopwn) by fangqyi.
...
1. Overwrite GLibC Malloc mmap chunk *size* or *prev_size*
1. Free the mmap chunk (with munmap) to small amount of memory mapping of part of LibC (`.gnu.hash`, `.dynsym`)
1. Get mmap chunk over the top of LibC region
1. Rewrite `.gnu.hash` and `.dynsym` sections of LibC ELF
1. Call previously uncalled function for code execution
...
1. Full RELRO or a static binary would prevent this attack from being successful because all symbols are loaded prior to the program running (no lazy symbol resolution)
1. Known **relative** location of the overwritten chunk. In some scenarios, this may need to be brute forced
1. The LibC (or other library) being attacked must be known or brute forced in order to have the offsets work properly
1. Fairly good control over the size of allocations in order to get mmap chunks in the proper locations

## Using the Repo

Upon some further expectation, it was noticed that the repo did not work AS is in all envs. For whatever reason  
...
So, it only seemed logical to create a quick & easy [Dockerfile](./Dockerfile) for this. 
...
- Once logged in the, the good stuff is in `house-of-muney`. So, move to this directly. 
- Jump into `tmux`: 
...
- Run the Code: `python launch.py`
  ...
  - If you are having issues with this, your offsets are probably broken; fix these yourself (they are likely in the intervals of page sizes). Good luck!
